...
{ref}`seaice` ([MPAS-Seaice](https://mpas-dev.github.io/sea_ice/sea_ice.html))
components. Land-ice support is planned but has not yet been migrated as its
own component.
...
you like. By default, steps run in sequence with `polaris serial`. Polaris also
includes an opt-in `polaris run` command path that builds a dependency graph
for the selected steps and records scheduler diagnostics. In the first phase of
the task-parallel rollout, `polaris run` still runs only one step at a time, so
it is primarily a validation and debugging path for future step-level
parallelism rather than a speedup feature. There is no requirement that all
steps defined in a task must run when that task is run. Some steps may be
disabled depending on config options (see {ref}`config-files`) that you choose.
Other steps, such as plotting or other forms of analysis, may be intended for
you to run them manually if you want to see the plots.
