...
class Init(OceanIOStep):
    """
    A step for an initial condition for for the external gravity
    wave test case
    """
    ...
    def run(self):
        ...
        gaussian_bump_value = gaussian_bump(
            lat_center, lon_center, latCell, lonCell
        )
        thickness_array, _ = xr.broadcast(
            gaussian_bump_value + bottom_depth, ds.refZMid
        )
...
def gaussian_bump(lat_center, lon_center, latCell, lonCell):
    ...
    """
    bump_value = np.exp(
        -100 * ((latCell - lat_center) ** 2 + (lonCell - lon_center) ** 2)
    )
